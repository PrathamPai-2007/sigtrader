...
from futures_analyzer.analysis.normalization import normalize_distension
...
def _detect_early_reversal_signals(
    ...
    *,
    ...
    # Requires either ≥5 bars since the cross OR price moved ≥0.1% from the cross bar.
...
class SetupAnalyzer:
    ...
    def analyze(
        ...
        *,
        ...
        # ── Side-specific bifurcation ─────────────────────────────────────────
        # Compute a shared indicator bundle for the baiter/sniper post-processors.
        _shared_bundle: IndicatorBundle | None = None
        try:
            _shared_bundle = compute_all_indicators(
                entry_candles, trigger_candles, context_candles, higher_candles,
                market, config=self._config,
            )
        except Exception:
            pass  # bundle is optional; gates degrade gracefully without it

        self._analyze_short_sniper(short_setup)
        if _shared_bundle is not None:
            self._analyze_long_baiter(long_setup, _shared_bundle, atr, px, trigger_candles)
        else:
            long_setup.entry_type = "limit"  # default even without bundle
    ...
    def _build_side(
        ...
        *,
        ...
        if _use_new_pipeline:
            ...
            # Apply per-call weight overrides (e.g. _analyze_long_baiter 1.5x boost)
            ...
            # --- Exhaustion penalty: crowded consensus suppresses confidence ---
            # Asymmetrical thresholds: shorts are penalised earlier (crowded moves reverse fast);
            # longs get more room to run with strength.
            ...
            if side == "short":
                _exhaustion_penalty = 0.15 if _overheated_count >= 5 else 0.0
            else:  # long
                _exhaustion_penalty = 0.05 if _overheated_count >= 7 else 0.0
            _exhaustion_adjusted_sum = _evidence.weighted_sum - _exhaustion_penalty
