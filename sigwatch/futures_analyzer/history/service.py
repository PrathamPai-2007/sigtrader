...
class HistoryService:
    ...
    async def _evaluate_snapshot(
        ...
        end_time = snapshot.as_of + timedelta(hours=24)
    ...
    def _evaluation_limit_for_interval(interval: str) -> int:
        ...
        if minutes is None or minutes <= 0:
            return 120
        return min(max(int((24 * 60) / minutes) + 2, 2), 1500)
    ...
    def recent_drawdown_state(
        ...
        *,
        ...
        snapshots = self.repository.evaluated(symbol=symbol)
        # Most-recent first; take up to n
        recent = snapshots[:n]
    ...
    def kelly_inputs(
        ...
        *,
        ...
        """
        snapshots = self.repository.evaluated(symbol=symbol)[:lookback]
        if not snapshots:
            return 0.5, 1.0, 1.0
