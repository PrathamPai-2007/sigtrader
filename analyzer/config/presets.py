...
class PresetProfile:
    ...
    Distinct from ``analyzer.config.PresetConfig`` (a Pydantic model loaded from
    JSON): this is a hard-coded strategy profile. ``PresetConfig`` is kept below as
    a backward-compatible alias for existing imports.
    """
    name: str
    description: str
    entry_timeframe: str
...
# Backward-compatible alias: external code imports ``PresetConfig`` from here.
PresetConfig = PresetProfile


# Pre-configured presets
PRESETS: dict[StrategyPreset, PresetProfile] = {
    StrategyPreset.SCALPER: PresetProfile(
        name="Scalper",
    ...
    StrategyPreset.POSITION_TRADER: PresetProfile(
...
def get_preset(preset: StrategyPreset | str) -> PresetProfile:
