...
from analyzer.config import DEFAULT_CONFIG_PATH, load_app_config, refresh_app_config
from analyzer.config_presets import get_preset, list_presets, StrategyPreset, PresetConfig
from analyzer.analysis.concurrency import ParallelAnalyzer, QueryOptimizer
from history import HistoryCompareBy, HistoryService
from providers import BinanceFuturesProvider
...
from market import SlippageAdvisor, CorrelationAnalyzer
from analyzer.logging import configure_logging, get_logger
from analyzer.analysis.scorer import DrawdownAdjuster
from portfolio import PortfolioRiskManager
from analyzer.reporting import render_drawdown_text, render_portfolio_text
...
async def _analyze_async(
    ...
    *,
    ...
    if preset_config is not None:
        from analyzer.analysis.models import TimeframePlan as _TFP
        timeframe_plan = _TFP(
...
def analyse(
    ...
    style: StrategyStyle = typer.Option(StrategyStyle.CONSERVATIVE, "--style", help="Strategy analysis style."),
    mode: MarketMode = typer.Option(MarketMode.INTRADAY, "--mode", help="Market regime timeframe mode."),
    ...
    as_json: bool = typer.Option(False, "--json", help="Output results in JSON format."),
    ...
    style, market_mode, preset_config = _resolve_preset(preset, style, mode)
...
def scan(
    ...
    mode: MarketMode = typer.Option(MarketMode.INTRADAY, "--mode", help="Market regime timeframe mode."),
    preset: StrategyPreset = typer.Option(StrategyPreset.POSITION_TRADER, "--preset", help="Trading strategy preset."),
    top: int = typer.Option(5, "--top", min=1, help="Maximum number of top-ranked setups to display."),
...
def find(
    top: int = typer.Option(5, "--top", min=1, help="Maximum number of top-ranked setups to display."),
    universe: int = typer.Option(20, "--universe", min=1, help="Size of liquid candidates universe to scan."),
    risk_reward: float | None = typer.Option(None, "--risk-reward", min=0.5, help="Fallback risk:reward."),
    style: StrategyStyle = typer.Option(StrategyStyle.AGGRESSIVE, "--style", help="Strategy analysis style."),
    mode: MarketMode = typer.Option(MarketMode.INTRADAY, "--mode", help="Market regime timeframe mode."),
    preset: StrategyPreset = typer.Option(StrategyPreset.POSITION_TRADER, "--preset", help="Trading strategy preset."),
    correlate: bool = typer.Option(False, "--correlate", help="Append correlation warnings for top-ranked setups."),
    capital: float | None = typer.Option(None, "--capital", min=1.0, help="Total capital in USD for portfolio allocation."),
...
def backtest(
    ...
    style: StrategyStyle = typer.Option(StrategyStyle.CONSERVATIVE, "--style", help="Strategy analysis style."),
    market_mode: MarketMode = typer.Option(MarketMode.INTRADAY, "--mode", help="Market regime timeframe mode."),
    preset: StrategyPreset | None = typer.Option(None, "--preset", help="Trading strategy preset."),
    risk_reward: float = typer.Option(2.0, "--risk-reward", min=0.5, help="Target risk:reward ratio."),
...
def slippage_cmd(
    ...
    entry: float = typer.Option(..., "--entry", help="Target entry price."),
    target: float = typer.Option(..., "--target", help="Target take-profit price."),
    stop: float = typer.Option(..., "--stop", help="Target stop-loss price."),
    order_size: float = typer.Option(1000.0, "--order-size", min=1.0, help="Position size in USD for slippage analysis."),
...
def correlate_cmd(
    symbols: str = typer.Option(..., "--symbols", help="Comma-separated Binance futures symbols."),
    interval: str = typer.Option("1h", "--interval", help="K-line timeframe interval (e.g. 1m, 5m, 1h, 1d)."),
    window: int = typer.Option(100, "--window", min=10, help="Number of historical bars for correlation calculation."),
    top: int = typer.Option(10, "--top", min=1, help="Number of top correlated pairs to display."),
    as_json: bool = typer.Option(False, "--json", help="Output results in JSON format."),
    export: Path | None = typer.Option(None, "--export", help="Write a .html, .md, or .txt report."),
) -> None:
...
def history_recent(
    ...
    limit: int = typer.Option(10, "--limit", min=1, help="Maximum number of recent records to display."),
    export: Path | None = typer.Option(None, "--export", help="Write a .html, .md, or .txt report."),
) -> None:
...
def history_stats(
    symbol: str | None = typer.Option(None, "--symbol", help="Filter to a specific symbol."),
    days: int | None = typer.Option(None, "--days", min=1, help="Number of historical days to analyze."),
    min_rsi: float | None = typer.Option(None, "--min-rsi", help="Filter by minimum RSI value."),
    max_rsi: float | None = typer.Option(None, "--max-rsi", help="Filter by maximum RSI value."),
    regime: str | None = typer.Option(None, "--regime", help="Filter by market regime classification."),
    min_ob_imbalance: float | None = typer.Option(None, "--min-ob-imbalance", help="Filter by minimum orderbook imbalance."),
    volatility_regime: str | None = typer.Option(None, "--volatility-regime", help="Filter by volatility regime classification."),
    export: Path | None = typer.Option(None, "--export", help="Write a .html, .md, or .txt report."),
) -> None:
...
def history_compare(
    by: HistoryCompareBy = typer.Option(..., "--by", help="Dimension to compare by (symbol, style, mode, confidence)."),
    symbol: str | None = typer.Option(None, "--symbol", help="Filter to a specific symbol."),
    days: int | None = typer.Option(None, "--days", min=1, help="Number of historical days to analyze."),
    export: Path | None = typer.Option(None, "--export", help="Write a .html, .md, or .txt report."),
) -> None:
...
def history_backfill_metrics(
    limit: int = typer.Option(500, "--limit", min=1, help="Maximum number of snapshots to backfill."),
) -> None:
