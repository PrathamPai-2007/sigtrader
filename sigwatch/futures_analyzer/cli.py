...
def _resolve_risk_reward(style: StrategyStyle, risk_reward: float | None) -> float:
    if risk_reward is not None:
        return risk_reward
    return load_app_config().style_tuning(style).fallback_risk_reward
...
async def _analyze_async(
    ...
    *,
    ...
    try:
        ...
        return result.model_copy(update={"chart_replay_last_tradable_at": replay_timestamp})
    finally:
        if owned_provider:
...
def analyse(
    ...
    if order_size is not None:
        try:
            ...
            cfg = load_app_config()
            model = SlippageModel(cfg.slippage.default_model)
            advisor = SlippageAdvisor(model=model, min_viable_rr=cfg.slippage.min_viable_rr)
...
def scan(
    ...
    if capital is not None and ranked_results:
        try:
            portfolio_report = _build_portfolio_report(ranked_results, capital, service, corr_clusters)
        except Exception as exc:
...
def find(
    ...
    fallback_top = load_app_config().find_fallback_top
...
def backtest(
    ...
    end: datetime = typer.Option(..., "--end", formats=["%Y-%m-%d", "%Y-%m-%dT%H:%M:%S"]),
    style: StrategyStyle = typer.Option(StrategyStyle.CONSERVATIVE, "--style"),
    market_mode: MarketMode = typer.Option(MarketMode.INTRADAY, "--mode"),
    preset: StrategyPreset | None = typer.Option(None, "--preset"),
