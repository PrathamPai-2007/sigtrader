...
from cli import _analyze_async, app
from history.models import HistorySnapshot, HistoryStatsReport, StatsBucket
...
class _FakeHistoryService:
    ...
    def compare(self, *, compare_by, symbol: str | None = None, days: int | None = None):
        ...
        from history.models import HistoryCompareReport
    ...
    def recent_drawdown_state(self, *, symbol: str | None = None, lookback: int | None = None):
        from history.models import DrawdownState
...
def test_cli_analyze_human(monkeypatch) -> None:
    ...
    monkeypatch.setattr("cli._analyze_async", fake_analyze_async)
    monkeypatch.setattr("cli._history_service", lambda: history)
...
def test_cli_analyze_json(monkeypatch) -> None:
    monkeypatch.setattr("cli._history_service", lambda: _FakeHistoryService())
...
def test_cli_scan_human(monkeypatch) -> None:
    ...
    monkeypatch.setattr("cli._history_service", lambda: history)
    monkeypatch.setattr("cli._scan_async", fake_scan_async)
...
def test_cli_find_human(monkeypatch) -> None:
    ...
    monkeypatch.setattr("cli._history_service", lambda: history)
    monkeypatch.setattr("cli._find_async", fake_find_async)
...
def test_structured_logger_accepts_keyword_fields() -> None:
    ...
    logger_name = "analyzer.tests.logging"
...
def test_analyse_async_keeps_owned_provider_open_through_replay(monkeypatch) -> None:
    ...
    monkeypatch.setattr("cli.BinanceFuturesProvider", lambda: provider)
