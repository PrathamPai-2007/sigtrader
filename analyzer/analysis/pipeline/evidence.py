...
from analyzer.config import AppConfig, StrategyConfig, load_app_config
from analyzer.analysis.scoring.utils import _clamp, _quality_label, _quality_score_cap_from_confidence
...
def _regime_weight_profile(regime: MarketRegime, side: str, config: AppConfig | None = None) -> tuple[dict[str, float], float, float]:
    """Return regime-specific weights, penalty multiplier, and confidence ceiling."""
    strategy = (config or load_app_config()).strategy
    ...
    penalty_multiplier = strategy.regime_penalty_multiplier.get(regime_key, strategy.regime_penalty_multiplier.get("default", 1.0))
    confidence_ceiling = strategy.regime_confidence_ceiling.get(side_key, strategy.regime_confidence_ceiling.get(regime_key, strategy.regime_confidence_ceiling.get("default", 1.0)))
    return weights, penalty_multiplier, confidence_ceiling


def _regime_alignment(regime: MarketRegime, side: str, config: AppConfig | None = None) -> float:
...
def _regime_penalty(regime: MarketRegime, side: str, config: AppConfig | None = None) -> float:
