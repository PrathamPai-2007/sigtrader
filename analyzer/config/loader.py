...
live in analyzer.config.json. Pydantic model defaults are only
...
from analyzer.analysis.models import MarketMode, StrategyStyle
from analyzer.logging import get_logger
...
class DeliberationSettings(BaseModel):
    range_agreement_threshold: int = 2
    range_confidence_mult: float = 0.90
    range_quality_penalty: float = 3.0
    default_agreement_mult: float = 0.90
    default_quality_penalty: float = 3.0
    edge_mult: float = 0.92
    macro_context_mult: float = 0.90
    macro_context_quality_penalty: float = 3.0
    trigger_entry_mult: float = 0.92
    chop_quality_cap: float = 50.0
    chop_agreement_threshold: int = 3
    chop_confidence_mult: float = 0.88
    chop_quality_penalty: float = 5.0
    range_quality_cap: float = 52.0
    range_low_quality_threshold: float = 45.0
    range_low_quality_penalty: float = 3.0


class BaiterSettings(BaseModel):
    rsi_fomo_threshold: float = 65.0
    bb_fomo_threshold: float = 0.75
    fomo_confidence_penalty: float = 0.30
    near_value_atr_dist: float = 0.2
    ema20_near_dist: float = 0.15
    out_of_value_penalty: float = 0.30
    red_candle_bonus: float = 0.05
    min_confidence_threshold: float = 0.60


class ExhaustionSettings(BaseModel):
    signal_threshold: float = 0.85
    short_penalty: float = 0.15
    short_count_limit: int = 5
    long_penalty: float = 0.05
    long_count_limit: int = 7
...
class StrategyConfig(BaseModel):
    ...
    allowed_regimes: list[str] = Field(
        default_factory=lambda: ["range", "bearish_trend", "bullish_trend", "breakout", "exhaustion", "volatile_chop", "transition"]
    )
    ...
    regime_penalty_multiplier: dict[str, float] = Field(
        default_factory=lambda: {"default": 1.0}
    )
    regime_confidence_ceiling: dict[str, float] = Field(
        default_factory=lambda: {"default": 1.0}
    )
    ...
    # ── Scoring Settings ──────────────────────────────────────────────────────
    deliberation: DeliberationSettings = Field(default_factory=DeliberationSettings)
    baiter: BaiterSettings = Field(default_factory=BaiterSettings)
    exhaustion_settings: ExhaustionSettings = Field(default_factory=ExhaustionSettings)
...
# NOTE: PresetConfig is defined once near the top of this module (with validators).
# A second, validator-less duplicate previously lived here and silently shadowed it,
# disabling config validation — it has been removed.
...
class AppConfig(BaseModel):
    ...
    market_modes: dict[str, TimeframeConfig] = Field(
        default_factory=lambda: {"intraday": TimeframeConfig()}
    )
    market_mode_tuning: dict[str, MarketModeTuning] = Field(
        default_factory=lambda: {"intraday": MarketModeTuning()}
    )
    styles: dict[str, StyleTuning] = Field(
        default_factory=lambda: {
            "conservative": StyleTuning(
                fallback_risk_reward=2.2,
                target_cap_atr_mult=3.0,
                ambition_penalty_start_atr=3.5,
                ambition_penalty_slope=2.0,
                min_confidence=0.6,
                min_quality=50.0,
                min_rr_ratio=1.5,
...
DEFAULT_CONFIG_PATH = Path.cwd() / "analyzer.config.json"
...
def ensure_default_config(path: Path = DEFAULT_CONFIG_PATH) -> Path:
    ...
    if not path.exists():
        from analyzer.config._template import MINIMAL_CONFIG
