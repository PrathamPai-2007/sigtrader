...
from analyzer.analysis import build_timeframe_plan
from analyzer.analysis.models import MarketMode, QualityLabel, StrategyStyle, TradeSetup
...
def test_default_repo_config_file_loads() -> None:
    ...
    path = Path("analyzer.config.json")
    assert path.exists()
...
def test_build_timeframe_plan_uses_loaded_config(monkeypatch) -> None:
    ...
    monkeypatch.setattr("analyzer.config.load_app_config", lambda: custom)
    refresh_app_config()
...
def test_repo_config_populates_new_strategy_sections() -> None:
    config_path = Path(__file__).resolve().parents[1] / "analyzer.config.json"
    refresh_app_config()
