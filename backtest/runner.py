...
from backtest.evaluator import resolve_outcome
from backtest.models import BacktestConfig, BacktestReport, BacktestTrade
from analyzer.config import DEFAULT_CONFIG_PATH, load_app_config
...
class BacktestRunner:
    ...
    def _build_timeframe_plan(self) -> TimeframePlan:
        from analyzer.config_presets import get_preset, StrategyPreset
        from analyzer.analysis.models import TimeframePlan as TFP
    ...
    def _run_in_memory(
        ...
        *,
        ...
        from backtest.evaluator import evaluation_window_for_timeframe
        ...
        for i, trigger_bar in enumerate(candidate_bars):
            ...
            if isinstance(ps, dict) and ps.get("mode") == "confidence_scaled":
                try:
                    from portfolio import get_position_size
            ...
            if self.config.order_size_usd is not None:
                try:
                    from history.evaluation import SlippageCalculator, SlippageModel
